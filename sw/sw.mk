...
CFLAGS  += -include $(BUILD)/install_paths.h
LDFLAGS += $(filter -fsanitize=% -pg,$(OPT))
LDLIBS  += $(RL_LIBS)

BIN  = $(BUILD)/cgra
OBJS = $(BUILD)/main.o $(BUILD)/dsl.o $(BUILD)/compile.o $(BUILD)/bench_ref.o

# Generate through make's file function: prefix text never becomes shell code.
# Compare before replacement so only a changed runtime path rebuilds objects.
escape_c = $(subst ",\",$(subst \,\\,$(1)))
define INSTALL_PATHS
/* Generated by sw.mk; excludes DESTDIR. */
#define CGRA_STDLIB_DIR "$(call escape_c,$(pkgdatadir)/stdlib)"
endef

.PHONY: force-install-paths
force-install-paths:

$(BUILD)/install_paths.h: force-install-paths | $(BUILD)
	$(file >$@.tmp,$(INSTALL_PATHS))
	@cmp -s $@.tmp $@ || cp $@.tmp $@
	@rm -f $@.tmp

$(OBJS): $(BUILD)/install_paths.h
...
.PHONY: all lib unit test smoke bench valgrind analyze gprof perf callgrind profile \
        profile-gprof profile-perf profile-callgrind doc info man clean
...
smoke: $(BIN)
	...
	./$(BIN) run add -d sim:v2 --a "1 2 3 4" --b "10 20 30 40"      # pre-v3 fallback
	./$(BIN) scan -d sim:v2 --a "3 1 4 1 5 9 2 6"                   # pre-v3 fallback
	...
	@test "$$(./$(BIN) run add -d sim:2x3 --a "1 2 3 4 5" --b "10 20 30 40 50")" = "11 22 33 44 55"
	@test "$$(./$(BIN) scan -d sim:3x2:v2 --a "3 1 4 1 5")" = "3 4 8 9 14"
	./$(BIN) probe -d sim:2x3
	./$(BIN) show add -d sim:3x2 >/dev/null
	@test "$$(./$(BIN) dump -d sim:1x16 | wc -w)" -eq 16

test: unit smoke
	sh test/test_cli.sh ./$(BIN)
	python3 test/test_bench.py ./$(BIN)
...
analyze: $(BUILD)/install_paths.h | $(BUILD)
	mkdir -p $(BUILD)/analyze
	@for src in main.c dsl.c compile.c bench_ref.c; do \
	    $(CC) -std=c11 -O1 -I$(LIBDIR)/include $(RL_CFLAGS) -include $(BUILD)/install_paths.h \
	        -fanalyzer -Wall -Wextra -Werror -c "$$src" \
	        -o "$(BUILD)/analyze/$$(basename "$$src" .c).o" || exit 1; \
	done
...
gprof:
	$(MAKE) -f sw.mk PROFILE=gprof profile-gprof
...
perf:
	$(MAKE) -f sw.mk PROFILE=perf profile-perf
...
callgrind:
	$(MAKE) -f sw.mk PROFILE=perf profile-callgrind

profile-callgrind: $(BIN)
